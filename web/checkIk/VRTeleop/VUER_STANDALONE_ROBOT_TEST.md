...
On this machine `so101_vuer` is already discovered. Use the optimized launcher
below. The later commands are retained only as troubleshooting fallbacks.

## 7. Run the optimized follower launcher (recommended)

This clone includes a launcher fixed to the documented follower port. It holds
the current pose until Quest tracking arrives and avoids the extra serial read
caused by combining the generic CLI loop with `max_relative_target`.

```bash
cd /home/quangduc/Code/VRTeleop/lerobot_teleoperator_so101_vuer
/home/quangduc/miniconda3/envs/lerobot/bin/python3 run_real_follower.py
```

When only the follower is connected, the launcher automatically uses the sole
`/dev/ttyACM*` device even if Linux renumbers it after a reconnect. With both
arms connected, explicitly pass the currently verified follower port, for
example `run_real_follower.py --port /dev/ttyACM1`. Stop it with `Ctrl+C`
before starting any other robot-control process.

## 7A. Run through `lerobot-teleoperate` when the plugin is discovered (fallback)
...
/home/quangduc/miniconda3/envs/lerobot/bin/lerobot-teleoperate \
  ...
  --teleop.vuer_cert=/home/quangduc/Code/VRTeleop/lerobot_teleoperator_so101_vuer/cert.pem \
  --teleop.vuer_key=/home/quangduc/Code/VRTeleop/lerobot_teleoperator_so101_vuer/key.pem \
  --fps=30
```
...
## 7B. Upstream fallback launcher when `so101_vuer` is missing from CLI help

This section is retained only for a separate clean clone. On this machine the
plugin is already discovered, so use the optimized launcher in section 7.
