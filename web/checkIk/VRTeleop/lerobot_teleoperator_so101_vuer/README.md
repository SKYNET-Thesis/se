...
On this laptop, use the guarded follower launcher from this repository:
...
When exactly one ACM device is connected, it is treated as the follower. When
both leader and follower are connected, the launcher refuses to guess; pass the
verified follower port explicitly with `--port`. Never copy a fixed ACM number
from an example because Linux can renumber the devices after reconnecting them.

The launcher maps motion relative to the headset heading: hand forward/back maps
to arm extend/retract, hand up/down maps to arm up/down, and pinching closes the
gripper. The operator does not need to face the same compass direction as the
robot.

The first valid hand frame is latched against the follower's measured end-effector
pose, so entering tracking does not make the arm jump. Subsequent commands use
hand displacement from that latch point rather than an absolute room coordinate.
